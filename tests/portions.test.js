...
t('the detail view renders a Portions pill', () => {
  assert.ok(/recipe\.portions.*Portions:/.test(script), 'no portions pill in the recipe detail view');
});
t('the prep layer renders one too', () => {
  assert.ok(/r\.portions.*Portions:/.test(script), 'no portions pill in the prep layer');
});
t('the editor has a Portions input bound to uF', () => {
  assert.ok(/uF\('portions',this\.value\)/.test(script), 'no editable portions field');
});
t('a brand-new recipe starts with an empty portions', () => {
  assert.ok(/portions:''/.test(script.replace(/\s/g, '')), 'new recipes omit portions');
});
...
t('the pill is conditional, so recipes without portions show nothing', () => {
  ...
  assert.strictEqual(guards.length, 2, `expected 2 guarded pills, found ${guards.length}`);
});
